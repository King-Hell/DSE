   ...
   // increase array length if necessary
	...
	{// double array length
	   // allocate a new array
		...
		if (start < 2)
			// no wrap around
			_Copy_unchecked(queue + start, queue + start + arrayLength - 1, newQueue);
		else
		{  // queue wraps around
			_Copy_unchecked(queue + start, queue + arrayLength, newQueue);
			_Copy_unchecked(queue, queue + theBack + 1, newQueue + arrayLength - start);
		}
